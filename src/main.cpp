...
// DistanceFilter logFilterNoGate(0.25f, 10000.0f, 255); //bộ lọc phụ trợ, ko gating
...
// //       display.setWiFiStatus(wifiConfig.ssid.c_str(), false);
...
// // void publishMeasurementMQTT(const Measurement& m, float dist_ema_med_m, float dist_gated_m, float fps) {
...
//         float dist_gated = display.getFilter(); //median+EMA+Gating
//         float dist_ema_med = logFilterNoGate.update(m.dist_m, true); //EMA phụ trợ
...
//         //publishMeasurementMQTT(m, dist_ema_med, dist_gated, fps); //update 14/11/2025
...
Controller app(lrf, display, btnFalling);
