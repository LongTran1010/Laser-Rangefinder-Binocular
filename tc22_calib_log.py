...
Author: Tran Thang Long - Dang Quoc Phu - cờ lốt
"""
import argparse
import csv
import json
...
CSV_COLUMNS = [
    ...
    "predict_hold_count", "fw_version", "schema_version",
...
def gather_metadata(args) -> dict:
    ...
    target_width = prompt_default("Target width (cm, chieu ngang)", 60, float)
    target_height = prompt_default("Target height (cm, chieu doc — QUAN TRONG cho pitch scan)",
                                   target_width, float)
    ...
    return {
        ...
        "target": {
            ...
            "width_cm": target_width,
            "height_cm": target_height,
            "angular_width_mrad": round(1000.0 * target_width / 100.0 / r0_nom, 3),
            "angular_height_mrad": round(1000.0 * target_height / 100.0 / r0_nom, 3),
            "shape": target_shape,
            "mount": tripod,
        },
        "range": {
